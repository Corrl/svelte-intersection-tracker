...
<script>
    ...
    const maxPercentageIds = derived(percentages, $percentages => {
    ...
    onMount(() => {
        ...
        const root = options.rootId && document.querySelector(`#${options.rootId}`) || null
        ...
        const fullOptions = {
            ...
            threshold: options.threshold ?? Array.from({length: fineness}, (_, i) => i / fineness).concat(1) // last item sometimes doesn't trigger with 1? >> .99?
        }

        const observer = new IntersectionObserver(handleIntersectionChange, fullOptions)

        Object.keys(elementsToObserve).forEach(group => {
            $intersecting[group] = {}
            $percentages[group] = {}
        })
        Object.values(elementsToObserve).flat().forEach(element => observer.observe(element))
